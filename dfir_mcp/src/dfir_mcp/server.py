"""dfir-mcp stdio server — speaks JSON-RPC 2.0 per the MCP spec.

Launch from Claude Code with:

    claude mcp add agentic-dfir python3 -m dfir_mcp.server
...
logging.basicConfig(
    level=logging.INFO,
    stream=sys.stderr,
    format="%(asctime)s [dfir-mcp] %(levelname)s %(message)s",
)
_log = logging.getLogger("dfir_mcp.server")
