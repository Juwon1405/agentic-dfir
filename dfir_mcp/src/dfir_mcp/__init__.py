"""
dfir-mcp — Custom MCP server exposing typed, read-only forensic functions.
...
# dfir_corr is a sibling package — extracted from dfir_mcp in v0.7.1.
# The supported install path is `pip install -e ./dfir_mcp -e ./dfir_corr`
...
# tree without installing either package. If you `pip install dfir-mcp`
# from a wheel without also installing dfir_corr the trick will not
# find the sibling, and correlate_* calls will raise ImportError. We
# don't list dfir-corr in pyproject.toml because the two packages live
# in the same repository and aren't published to PyPI separately.
_DFIR_CORR_SRC = Path(__file__).resolve().parent.parent.parent.parent / "dfir_corr" / "src"
if _DFIR_CORR_SRC.exists() and str(_DFIR_CORR_SRC) not in sys.path:
    sys.path.insert(0, str(_DFIR_CORR_SRC))

EVIDENCE_ROOT = Path(os.environ.get("DFIR_EVIDENCE_ROOT", "/mnt/evidence"))
...
def _validate_arguments(tool_name: str, schema: dict, arguments: dict) -> dict:
    """Validate the subset of JSON Schema used by dfir-mcp tool schemas.
...
def call_tool(name, arguments):
    if name not in _REGISTRY:
        raise KeyError(f"ToolNotFound: '{name}' is not exposed by dfir-mcp")
    spec = _REGISTRY[name]
...
def correlate_events(hypothesis_id, usb_events=None, logon_events=None,
    ...
    # Delegates to dfir_corr.correlate_events. The MCP wire surface
    # (this function's name, schema, and registration) is unchanged;
    # only the implementation moved out of dfir_mcp into the
    # extracted package in v0.7.1.
    from dfir_corr import correlate_events as _impl
...
def correlate_timeline(events, rules=None, window_seconds=300):
    # Core correlation is now in dfir_corr. The MCP layer keeps two
    # things on top:
    ...
    #      to DuckDB).
    from dfir_corr import correlate_timeline as _impl
    ...
    # Map dfir_corr's shape into the back-compat result shape that
...
# native (`log show`, `fsevents-parser`, sqlite3). Agentic-DFIR consumes their
...
# link back to (1), and (4). Without these, an analyst using Agentic-DFIR
...
# ── Linux cron jobs ────────────────────────────────────────────────
# Implemented in dfir_mcp._v06_macos_linux as parse_linux_cron_jobs
...
#
# Agentic-DFIR previously covered post-auth behavior (analyze_windows_logons,
...
def detect_ransomware_behavior(processes=None, fsevents_or_mft=None,
    ...
    for p in processes:
        ...
        # would expand coverage but shift the baseline; deferred, tracked in
        # issue #43.
...
@tool(
    ...
    schema={"type": "object", "properties": {
        "hive_path": {"type": "string",
                      "description": "Relative path within DFIR_EVIDENCE_ROOT "
                                     "to the hive file."},
        "key": {"type": "string",
...
def parse_registry_hive(hive_path, key, value_name=None, limit=100):
    ...
    # python-registry is the read-only parser. Import lazily so dfir_mcp
...
# Plaso. Wrapped in read-only EVIDENCE_ROOT sandbox + SHA-256 audit + timeout
# guards. Adapters fail loudly via SiftToolNotFoundError when their tool isn't
