...
DERIVED_ROOT_ENV = "DFIR_DERIVED_ROOT"


def _log2timeline_bin() -> str:
    return _which("log2timeline.py", env_var="DFIR_LOG2TIMELINE_BIN")


def _psort_bin() -> str:
    return _which("psort.py", env_var="DFIR_PSORT_BIN")
...
def _derived_root() -> Path:
    ...
    return (Path(tempfile.gettempdir()) / "agentic-dfir-derived").resolve()


def _safe_derived_path(path_str: str, *, must_exist: bool = False) -> Path:
    """Resolve a Plaso storage path under DFIR_DERIVED_ROOT.
...
@tool(
    ...
    schema={
        ...
        "properties": {
            ...
            "output_storage_path": {
                "type": "string",
                "description": "Relative or absolute path under DFIR_DERIVED_ROOT for the .plaso file",
            },
            "parsers": {
                "type": "string",
...
def sift_plaso_psort(
    ...
    with _tempdir(prefix="dfir-psort-") as workdir:
