...
def _mftecmd_bin() -> str:
    return _which("MFTECmd", env_var="DFIR_MFTECMD_BIN")
...
def sift_mftecmd_parse(mft_path: str, limit: int = 5000) -> dict[str, Any]:
    ...
    with _tempdir(prefix="dfir-mftecmd-") as workdir:
...
def sift_mftecmd_timestomp(
    ...
    for row in parsed["rows"]:
        ...
        # finding count for the demo harness. Deferred so the release ships
        # with a stable baseline (see issue #43).
