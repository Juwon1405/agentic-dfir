...
Custom MCP server that exposes typed, schema-validated, **read-only** forensic functions to Claude Code. The primary enforcement layer for Agentic-DFIR's evidence integrity guarantee.
...
readers. Evidence inputs are constrained to `DFIR_EVIDENCE_ROOT`; Plaso
storage outputs are constrained to `DFIR_DERIVED_ROOT` so generated timelines
