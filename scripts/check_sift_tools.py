...
def main() -> int:
    ...
    if args.json:
        ...
        if missing:
            ...
            print("  - YARA / Volatility3 / Plaso: bash scripts/install.sh --full")
            print("    (or apt/pip per your distro)")
            print("  - EZ Tools (MFTECmd/EvtxECmd/PECmd/RECmd/AmcacheParser):")
            print("    bash scripts/install.sh --install-eztools")
