...
└── tests/
    └── test_dfir_corr.py      # 14 tests; pytest dfir_corr/tests/
```
...
Via the MCP wire (what the agent uses): the same functions are re-exported through `dfir_mcp` with name/schema preserved for backwards compatibility. `dfir_mcp.correlate_timeline` adds a thin SQL-injection defense layer on top before delegating to `dfir_corr.correlate_timeline`.
...
Tests run independent of `dfir_mcp`. The dfir_mcp wrappers exist only to expose these functions on the MCP wire; the engine itself has no MCP coupling.
