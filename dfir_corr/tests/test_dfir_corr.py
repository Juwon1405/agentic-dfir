"""Tests for dfir_corr — the extracted correlation engine.

These exercise dfir_corr in isolation, without dfir_mcp wrapping.
Same inputs as the dfir_mcp surface tests must produce identical
outputs (the dfir_mcp wrappers are thin pass-throughs)."""
...
def test_correlate_events_flags_ip_kvm_before_logon():
    ...
    r = dfir_corr.correlate_events("h_001", usb, logon, proximity_seconds=600)
...
def test_correlate_events_ignores_non_ip_kvm_usb():
    ...
    r = dfir_corr.correlate_events("h_002", usb, logon)
...
def test_correlate_events_respects_proximity_window():
    ...
    r = dfir_corr.correlate_events("h_003", usb, logon, proximity_seconds=600)
...
def test_correlate_events_handles_malformed_timestamps():
    ...
    r = dfir_corr.correlate_events("h_004", usb, logon)
...
def test_correlate_timeline_empty_input():
    r = dfir_corr.correlate_timeline([])
...
def test_correlate_timeline_finds_cross_source_actor_match():
    ...
    r = dfir_corr.correlate_timeline(events, window_seconds=60)
...
def test_load_rules_handles_missing_file():
    r = dfir_corr.load_rules(Path("/nonexistent/path/rules.yaml"))
    assert r["rules"] == []


# ─── Module-level invariants ──────────────────────────────────────────────

def test_dfir_corr_version_is_wellformed():
    # Version is sourced from package metadata (pyproject.toml), NOT pinned here,
    # so a release bump never requires editing this test. Assert it's a real,
    # well-formed semver-ish string rather than the unknown-fallback.
    v = dfir_corr.__version__
    assert v and v != "0.0.0+unknown"
    core = v.split("+")[0].split(".")
    assert len(core) >= 3 and all(p.isdigit() for p in core[:3])
...
def test_public_api_surface_is_complete():
    ...
    assert expected.issubset(set(dfir_corr.__all__))
