"""
dfir-agent — Iteration controller with senior-analyst loop and self-correction.

This is the wrapper that sits between Claude Code and dfir-mcp. It:
...
class DeterministicAnalyst:
    ...
    flagging are ALL the actual logic that dfir-agent runs. What's
    ...
    def _call(self, tool_name: str, inputs: dict, *,
        ...
        so the audit chain is queryable via `dfir-audit trace <fid>`.
        """
    ...
    def _phase_timeline(self) -> None:
        self.iteration += 1
        # Pre-declare F-001 on the audit entry so `dfir-audit trace F-001`
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(prog="dfir-agent",
                                 description="Agentic-DFIR autonomous DFIR agent")
    ...
    ap.add_argument("--mode", choices=["deterministic", "live"], default="deterministic",
                    help="deterministic: scripted analyst (for demo/accuracy). "
                         "live: Claude API talking to dfir-mcp via stdio.")
    ...
    ap.add_argument("--model",
                    ...
                    help="(live mode) Anthropic model id. Defaults to claude-haiku-4-5-20251001. Override via DFIR_MODEL env or --model (e.g. claude-sonnet-4-6 for higher fidelity).")
    ...
    print(f"[dfir-agent] case={args.case}")
    print(f"[dfir-agent] iterations: {report['iterations']}")
    print(f"[dfir-agent] findings: {len(report['findings'])}")
    print(f"[dfir-agent] audit chain: {msg}")
    print(f"[dfir-agent] outputs in: {args.out}")
