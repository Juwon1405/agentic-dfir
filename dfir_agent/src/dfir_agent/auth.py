...
def build_anthropic_client(model: str | None = None,
    ...
    if mode == "oauth":
        ...
        try:
            ...
            log.debug("[dfir-auth] oauth refresh attempt failed (ignored): %s", e)
        log.info("[dfir-auth] %s → local Claude credentials (oauth, source: %s)",
    ...
    if mode == "api":
        log.info("[dfir-auth] %s → ANTHROPIC_API_KEY (api)", short)
        return (anthropic.Anthropic(timeout=timeout, max_retries=max_retries), "api")

    log.warning("[dfir-auth] no credential source available; client cannot be built")
    return None, None
