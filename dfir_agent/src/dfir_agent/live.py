"""Live-mode controller for dfir-agent.

Connects Claude to dfir-mcp over a stdio subprocess. Claude sees only the
typed forensic tools registered in dfir-mcp; it cannot execute arbitrary
code because there is no execute_shell for it to call.

Authentication is resolved by dfir_agent.auth. The documented path for real
...
Default model is claude-haiku-4-5-20251001. Override with --model or DFIR_MODEL env:
    python3 -m dfir_agent --mode live --model claude-sonnet-4-6 ...
...
# dfir_playbook/, not a string baked into this file. We load the latest
...
# ---------------------------------------------------------------------------
_PLAYBOOK_DIR = Path(__file__).resolve().parents[3] / "dfir_playbook"
...
SYSTEM_PROMPT = """You are Agentic-DFIR, a senior DFIR analyst.

You have access to a set of typed, read-only forensic functions exposed by
the dfir-mcp server. These functions are the ONLY way you can interact
...
async def _run_with_real_claude(prompt: str, state: LiveRunState,
    ...
    Uses the configured Anthropic credential source from dfir_agent.auth.
    """
    from .auth import build_anthropic_client
    client, _auth_mode = build_anthropic_client(model)
    if client is None:
...
async def live_run(case: str, out_dir: str, prompt: str,
    ...
    # Launch dfir-mcp as a stdio subprocess
    ...
    async with AsyncExitStack() as stack:
        # Open stdio transport to dfir-mcp
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="dfir-agent live-mode controller")
    ...
    ap.add_argument("--model",
                    default=os.environ.get("DFIR_MODEL", "claude-haiku-4-5-20251001"))
