...
| **Realistic (noise-injected)** | `examples/sample-evidence-realistic/` | Hand-curated production volume (security EventLog ~11,530 lines, supply-chain 427, RDP brute 452, USB setupapi 107, memory triage, etc.); the two IOC-only logs are enriched with benign noise — web access 1027 lines (1:37), unix auth 517 (1:29) | Demonstrates needle-in-haystack recall. Benign noise on the two IOC-only logs generated by `scripts/generate_realistic_evidence.py` (deterministic seed = 20260508); all other evidence is committed hand-curated. |

Both variants score **identically** on the case-01 reference findings (F-001, F-013) —
