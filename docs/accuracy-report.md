...
|---|---|---|---|
| **Canonical bundled** | `examples/case-studies/self-evaluation/case-01/evidence_root/` | Hand-curated production volume (security EventLog ~11,530 lines, supply-chain 427, RDP brute 452, USB setupapi 107, memory triage, etc.); the two IOC-only logs are enriched with benign noise — web access 1027 lines (1:37), unix auth 517 (1:29) | The scored evidence. Needle-in-haystack recall. Benign noise on the two IOC-only logs is committed as-is (deterministic, seeded); all other evidence is committed hand-curated. |

`scripts/eval/demo.py` scores the canonical bundled tree on the case-01 findings
(F-001, F-013) — recall=1.000, FPR=0.000, hallucination=0 — confirming that the
...
Every evidence file in the canonical bundled tree now matches the
...
Schema-level fidelity is byte-stable per re-run; the full pytest suite and
...
| Evidence integrity preserved | **true** (62 reference files / 67 realistic files, SHA-256 pre/post match) |
...
3. **Volatility memory forensics** runs through the 12 `sift_vol3_*` adapters when
   Volatility 3 is installed; the native fallback reads the process tree and
   sockets only.
...
Layer 1 (synthetic, noise-injected ~1:30, 8 cases): **69 findings**.
...
~50-100% IOC by line). The realistic variant carries the same ground-truth
IOCs inside hand-curated production-volume evidence, with the two IOC-only
logs (web access, unix auth) enriched to ~1:30 benign noise — and the
headline numbers do not change. This rules out
