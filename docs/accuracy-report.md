...
|---|---|---|---|
| **CI fixture (toy)** | `examples/sample-evidence/` | ≤30 lines/file, fully IOC-loaded | Byte-stable fixture used by the unit tests. Not a user-selectable evidence set. |
| **Canonical bundled** | `examples/case-studies/self-evaluation/case-01/evidence_root/` | Hand-curated production volume (security EventLog ~11,530 lines, supply-chain 427, RDP brute 452, USB setupapi 107, memory triage, etc.); the two IOC-only logs are enriched with benign noise — web access 1027 lines (1:37), unix auth 517 (1:29) | The scored evidence. Needle-in-haystack recall. Benign noise on the two IOC-only logs generated by `scripts/generate_realistic_evidence.py` (deterministic seed = 20260508); all other evidence is committed hand-curated. |
