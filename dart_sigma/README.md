# dart_sigma — consolidated Sigma detection pack (versioned)
...
| | role | when it acts |
|---|---|---|
| **Playbook** (`dart_playbook/`) | the *investigation methodology* — phase order, which tools to call, when to form hypotheses, plus technique-classification guidance | drives the agent loop end-to-end |
| **Sigma pack** (`dart_sigma/`) | *detection signatures* — point patterns that flag a specific known-bad event shape | applied to parsed events by the `match_sigma_rules` MCP tool |
...
`match_sigma_rules` (registered in `dart_mcp`, native tool) loads this pack and
...
Note: this is different from `dart_corr/correlation-rules.yaml`, which drives
