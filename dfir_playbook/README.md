...
5. Run with `python3 -m dfir_agent --playbook dfir_playbook/<your-name>-v1.yaml`
...
6. **Findings cite audit_ids.** Always. Every finding carries the audit_ids of the MCP calls that produced it, and `python3 -m dfir_audit trace <audit.jsonl> <finding-id>` walks the chain back to every entry that produced it.
